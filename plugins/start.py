        ...
        try:
            buttons.append([
                InlineKeyboardButton(
                    text='♻️ Retry Get File',
                    callback_data=f"retry_fsub:{message.command[1]}"
                )
            ])
        except IndexError:
            pass
...
@Bot.on_callback_query(filters.regex(r"^retry_fsub:(.+)"))
async def retry_fsub(client: Client, query: CallbackQuery):
